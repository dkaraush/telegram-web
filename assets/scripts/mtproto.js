...
var protos = [];
load('assets/tl.json', function (status, schema) {
	if (!status) {
...
var MTProto = Class.extend({
	...
	listenUpdates: function (func) {
		var id = ~~(Math.random() * 9999);
		this.updateListeners[id] = func;
		return id;
	},
	forgetUpdates: function (id) {
		if (this.updateListeners[id])
	...
	onMessage: function (msg, msg_id) {
		if (this.auth_key != null) {
			if (msg._ == 'rpc_result') {
				...
				this.save({server_salt: this.server_salt});
				this.acks.push(msg_id);
			} else if (msg._.slice(0, 6) == 'update') {
				for (var k in this.updateListeners)
					this.updateListeners[k](msg);
				this.acks.push(msg_id);
			}
