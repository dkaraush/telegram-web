var bigInt = function(undefined) {
    ...
    BigInteger.prototype.modPow = function(b, n, cb) {
        b = parseValue(b);
        n = parseValue(n);
        if (b.isZero()) throw new Error("Cannot take modPow with modulus 0");
        var a = this.mod(n), r = bigInt(1), j = 0, x = a;
        // if (cb) {
     //    	function sub() {
     //    		if (b.compare(0) <= 0) {
     //    			cb(r);
     //    		} else {
     //    			console.log(j)
     //    			for (var i = 0; i < 100 && b.compare(0) > 0; ++i) {
     ...
     //    	return;
        // } else {
			while (b.isPositive()) {
			    if (a.isZero()) return Integer[0];
			    if (b.isOdd()) r = r.multiply(a).mod(n);
			    b = b.divide(2);
			    a = a.square().mod(n)
			}
		// }
